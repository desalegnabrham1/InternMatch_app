...
class InternshipDetailScreen extends StatelessWidget {
  ...
  Future<void> _launchEmail(BuildContext context) async {
    final uri = Uri(
      scheme: 'mailto',
      path: internship.contactEmail,
      queryParameters: {
        'subject': 'Internship Application – ${internship.title}',
        'body':
            'Hello,\n\nI am interested in the ${internship.title} internship position at ${internship.companyName}.\n\nPlease find my details attached.\n\nThank you.',
      },
    );
  ...
  Future<void> _launchPhone(BuildContext context) async {
    final uri = Uri(scheme: 'tel', path: internship.contactPhone);
    if (await canLaunchUrl(uri)) {
      await launchUrl(uri);
    } else {
      if (context.mounted) {
